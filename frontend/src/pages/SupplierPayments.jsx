...
import { FaMoneyBillWave, FaSearch, FaFilter, FaBuilding, FaChevronLeft, FaChevronRight, FaCreditCard, FaMobileAlt, FaUniversity, FaFileInvoiceDollar } from "react-icons/fa";
...
export default function SupplierPayments() {
  ...
  const filteredPayments = useMemo(() => {
    ...
    data = data.filter(p => {
      const text = `${p.payment_id} ${p.po_id} ${p.payment_method} ${p.transaction_id}`.toLowerCase();
    ...
    data.sort((a, b) => {
      let valA = a[sortKey];
      let valB = b[sortKey];
      if (["payment_id", "po_id", "amount"].includes(sortKey)) { valA = Number(valA); valB = Number(valB); }
  ...
  const stats = useMemo(() => {
    const totalOutflow = filteredPayments.reduce((sum, p) => sum + Number(p.amount), 0);
    const methodCounts = { NetBanking: 0, Cash: 0 };
    filteredPayments.forEach(p => { if (methodCounts[p.payment_method] !== undefined) methodCounts[p.payment_method]++; });
    return { total: filteredPayments.length, totalOutflow, ...methodCounts };
  }, [filteredPayments]);
    ...
    <DashboardLayout>
      <div className="max-w-[1600px] mx-auto animate-fade-in pb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
          <div>
            <h1 className="text-4xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Outflow Audit</h1>
            <p className="text-slate-500 dark:text-slate-400 font-bold text-sm mt-1 uppercase tracking-widest">Supplier settlements & expense logs</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="px-5 py-2.5 rounded-2xl bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 font-black text-xs uppercase tracking-widest border border-rose-100 dark:border-rose-800/30 shadow-sm">
              Total Outflow: ₹{stats.totalOutflow.toLocaleString("en-IN")}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <MetricCard title="Total Settlements" value={stats.total} icon={FaFileInvoiceDollar} color="rose" />
          <MetricCard title="Capital Disbursed" value={`₹${stats.totalOutflow.toLocaleString("en-IN")}`} color="rose" />
          <MetricCard title="Net Banking" value={stats.NetBanking} icon={FaUniversity} color="indigo" />
          <MetricCard title="Cash Payments" value={stats.Cash} icon={FaMoneyBillWave} color="amber" />
        </div>
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-xl rounded-[2rem] border border-slate-100 dark:border-slate-700/50 p-8 shadow-2xl shadow-slate-200/50 dark:shadow-none mb-8">
          <div className="flex flex-col lg:flex-row items-center gap-6">
            <div className="relative flex-1 group">
              ...
              <input
                type="text"
                placeholder="Find TXN, PO ID or Method..."
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-900/50">
                  ...
                  <th className={thClass} onClick={() => handleSort("po_id")}>PO Ref {sortKey === "po_id" && (sortOrder === "asc" ? "↑" : "↓")}</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {paginatedPayments.map((p) => {
                  const config = methodConfig[p.payment_method] || { color: "slate", icon: FaMoneyBillWave };
                  const MethodIcon = config.icon;
                  return (
                    <tr key={p.payment_id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-all duration-200">
                      <td className="px-6 py-5 font-black text-xs text-rose-500">#{p.payment_id}</td>
                      <td className="px-6 py-5">
                        <div className="font-bold text-slate-800 dark:text-white uppercase text-xs tracking-widest">PO-{p.po_id}</div>
                      </td>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-12 gap-6 pb-10">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
            Auditing {filteredPayments.length} verified supplier settlements
          </p>
